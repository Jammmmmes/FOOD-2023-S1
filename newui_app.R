                      ...
                      fluidRow(
                        column(2,
                               wellPanel(
                                 verticalLayout(
                                   h4("Select Options"),
                                   selectInput('providerName', "Map", choices = providers),
                                   selectInput('countries', "Countries", choices = sort(as.character(pgsp@data$admin) %>% unique)),
                                   tags$hr(),
                                   tags$h4("Function Introduction"),
                                   tags$p("This application allows users to view the corresponding map by selecting a country from the dropdown menu. Additionally, the program can predict future vegetation indices based on remote sensing data.")
